...
class CanalblockPlanController extends Controller
{
    use ControllerTrait;
    ...
    public function geoJson($province_id) {

        $q = CanalblockPlan::select('lat','lng')->where('province_id', '=', $province_id)->limit(8000)->get()->toArray();

        $coordinates = array();

        foreach($q as $key => $value) {
            $coordinates[] = array($value['lat'], $value['lng']);
        }
        $new_data = array(
            'type' => 'FeatureCollection',
            'features' => array(array(
                'type' => 'Feature',
                'geometry' => array('type' => 'MultiPoint', 'coordinates' => $coordinates),
                'properties' => array('kegiatan' => 'Sekat Kanal'),
            )),
        );
