<?php

use Illuminate\Database\Seeder;

class OrganisasiTableSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        DB::table('organisasi')->insert([
            'id' => 1,
           'country'=> 'Norway',
           'name'=> 'Kingdom of Norway',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 2,
           'country'=> 'Norway',
           'name'=> 'DFID UK Climate Change Unit Indonesia  (UKCCU)',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 3,
           'country'=> 'Japan',
           'name'=> 'Japan',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 4,
           'country'=> 'Japan',
           'name'=> 'Millenium Challenge Corporation',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 5,
           'country'=> 'Japan',
           'name'=> 'Norway',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 6,
           'country'=> 'Japan',
           'name'=> 'French Government (shareholder) ; financial markets (loans) ; delegated funds (grants)',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 7,
           'country'=> 'Japan',
           'name'=> 'Department of Foreign Affairs and Trade (DFAT)',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 8,
           'country'=> 'Japan',
           'name'=> 'Climate Works Foundation',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 9,
           'country'=> 'Japan',
           'name'=> 'Ford Foundation',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 10,
           'country'=> 'Japan',
           'name'=> 'Packard Foundation',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 11,
           'country'=> 'Japan',
           'name'=> 'Good Energies Foundation',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 12,
           'country'=> 'Japan',
           'name'=> 'UKCCU',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 13,
           'country'=> 'Japan',
           'name'=> 'Government of Norway/Embassy of Norway',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 14,
           'country'=> 'Japan',
           'name'=> 'World Economic Forum',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 15,
           'country'=> 'Japan',
           'name'=> 'Embassy of the Kingdom of the Netherlands',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 16,
           'country'=> 'Japan',
           'name'=> 'APP Group ',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 17,
           'country'=> 'Japan',
           'name'=> 'Global Affairs Canada',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 18,
           'country'=> 'Japan',
           'name'=> 'Climate and Land Use Alliance (CLUA)',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 19,
           'country'=> 'Japan',
           'name'=> 'Japan',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 20,
           'country'=> 'Japan',
           'name'=> 'MCA-I',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 21,
           'country'=> 'Japan',
           'name'=> 'Republic of South Korea',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 22,
           'country'=> 'Japan',
           'name'=> 'Ministry of Foreign Affairs of Denmark',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 23,
           'country'=> 'Japan',
           'name'=> 'EC (European Commission)',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 24,
           'country'=> 'Japan',
           'name'=> 'Federal Ministry for Economic Cooperation and Development (grant)',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 25,
           'country'=> 'Japan',
           'name'=> 'WWF Network',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 26,
           'country'=> 'Japan',
           'name'=> 'EMM',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 27,
           'country'=> 'Japan',
           'name'=> 'Walhi Jambi, Scale Up, UNDP, EMM, SAFIR, Madani',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 28,
           'country'=> 'Japan',
           'name'=> 'UNDP, TFCa, MCA, Belantara, IDH (Inisiasi Dagang Hijau), ICCTF (for on going project only ICCTF and Belantara)',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 29,
           'country'=> 'Japan',
           'name'=> 'European Union, International Fund for Agricultural Development (IFAD), Global Environment Facility (GEF) through IFAD',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 30,
           'country'=> 'Japan',
           'name'=> 'Royal Norwegian Embassy',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
           'id' => 31,
           'country'=> 'Japan',
           'name'=> 'Kingdom of Denmark & Norway',
           'upload'=> 'Lembaga',
           'key'=>'2323',
           'focal'=> '12',
       ]);
       DB::table('organisasi')->insert([
        'id' => 32,
        'country'=> 'Japan',
        'name'=> 'Indonesia Climate Change Trust Fund',
        'upload'=> 'Lembaga',
        'key'=>'2323',
        'focal'=> '12',
        ]);
        DB::table('organisasi')->insert([
         'id' => 33,
         'country'=> 'Japan',
         'name'=> 'United Nations Development Programme',
         'upload'=> 'Lembaga',
         'key'=>'2323',
         'focal'=> '12',
         ]);
       DB::table('organisasi')->insert([
        'id' => 34,
        'country'=> 'Japan',
        'name'=> 'Japan International Cooperation Agency',
        'upload'=> 'Lembaga',
        'key'=>'2323',
        'focal'=> '12',
        ]);
        
       DB::table('organisasi')->insert([
        'id' => 35,
        'country'=> 'Japan',
        'name'=> 'Millenium Challenge Account Indonesia',
        'upload'=> 'Lembaga',
        'key'=>'2323',
        'focal'=> '12',
        ]);
        
       DB::table('organisasi')->insert([
        'id' => 36,
        'country'=> 'Japan',
        'name'=> 'Forum Indonesia Untuk Transparansi Anggaran - Riau',
        'upload'=> 'Lembaga',
        'key'=>'2323',
        'focal'=> '12',
        ]);
        
       DB::table('organisasi')->insert([
        'id' => 37,
        'country'=> 'Japan',
        'name'=> 'Wahana Lingkungan Hidup Indonesia - Sumsel',
        'upload'=> 'Lembaga',
        'key'=>'2323',
        'focal'=> '12',
        ]);
        
       DB::table('organisasi')->insert([
        'id' => 38,
        'country'=> 'Japan',
        'name'=> 'Institute Agroekologi Indonesia',
        'upload'=> 'Lembaga',
        'key'=>'2323',
        'focal'=> '12',
        ]);
        
       DB::table('organisasi')->insert([
        'id' => 39,
        'country'=> 'Japan',
        'name'=> 'World Wide Fund for Nature',
        'upload'=> 'Lembaga',
        'key'=>'2323',
        'focal'=> '12',
        ]);
               
      

    }
}
